                    ...
                    <tr class="group hover:bg-blue-50/30 dark:hover:bg-blue-900/5 transition-colors duration-150">
                        ...
                        <td class="px-5 py-3.5 text-center">
                            <div class="relative" x-data="{ open: false }">
                                ...
                                <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 z-50 mt-1 w-52 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 py-1" style="display: none;">
                                    <a href="{{ route('dosen.kursus.progres.detail', ['id' => $enrollment->course->id_course, 'enrollmentId' => $enrollment->id_enroll]) }}" class="flex items-center gap-2.5 px-3.5 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                        Lihat Detail
                                    </a>
                                    @if($enrollment->final_assignment_submission)
                                    <a href="{{ route('dosen.assignment-submissions.preview', $enrollment->final_assignment_submission['id']) }}" target="_blank" rel="noopener" class="flex items-center gap-2.5 px-3.5 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h6m-8 8h14a2 2 0 002-2V6a2 2 0 00-2-2H8l-5 5v9a2 2 0 002 2z"/></svg>
                                        {{ $enrollment->final_assignment_submission['is_final_assignment'] ? 'Lihat Tugas Akhir' : 'Lihat Berkas Tugas' }}
                                    </a>
                                    @endif
