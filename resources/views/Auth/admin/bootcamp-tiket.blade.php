...
<x-layouts.admin title="Bootcamp & Tiket" active="bootcamp">
                            ...
                            <div class="mt-4 grid gap-3 md:grid-cols-4">
                                ...
                                <div class="rounded-2xl bg-slate-50 p-4">
                                    ...
                                    <div class="mt-2 flex flex-wrap gap-2">
                                        ...
                                        @if (!empty($program['linked_course_id']))
                                            <a href="{{ route('admin.bootcamp.sesi.show', ['courseId' => $program['linked_course_id']], false) }}" class="rounded-xl border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-semibold text-indigo-700 hover:bg-indigo-100">Kelola Sesi</a>
                                        @else
                                            <button type="button" disabled title="Buka penjualan bootcamp dulu (linked course belum dibuat)" class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-400 cursor-not-allowed">Kelola Sesi</button>
                                        @endif
    ...
    <div id="bootcampCreateModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        ...
        <div class="relative flex min-h-full items-start justify-center px-4 py-4 sm:items-center sm:py-6">
            <div class="flex w-full max-w-3xl max-h-[calc(100dvh-2rem)] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
                ...
                <form method="POST" action="{{ route('admin.bootcamp-tiket.store', [], false) }}" enctype="multipart/form-data" class="flex-1 space-y-5 overflow-y-auto px-6 py-6">
                    @csrf
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <label class="block"><span class="mb-2 block text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Gambar Cover</span><input name="thumbnail" type="file" accept="image/jpeg,image/png,image/webp" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm file:mr-3 file:rounded-xl file:border-0 file:bg-blue-100 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-blue-700"><p class="mt-1 text-xs text-slate-500">Opsional. JPG, PNG, atau WebP, maksimal 5 MB.</p></label>
    ...
    <div id="bootcampBatchModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        ...
        <div class="relative flex min-h-full items-start justify-center px-4 py-4 sm:items-center sm:py-6"><div class="flex w-full max-w-xl max-h-[calc(100dvh-2rem)] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
            ...
            <form id="bootcampBatchForm" method="POST" enctype="multipart/form-data" class="flex-1 space-y-5 overflow-y-auto px-6 py-6">
                @csrf @method('PUT')
                <div class="grid gap-4 sm:grid-cols-2">
                ...
                <label class="block"><span class="mb-2 block text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Ganti Gambar Cover</span><input name="thumbnail" type="file" accept="image/jpeg,image/png,image/webp" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm file:mr-3 file:rounded-xl file:border-0 file:bg-blue-100 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-blue-700"><p class="mt-1 text-xs text-slate-500">Kosongkan jika gambar lama ingin dipertahankan. Maksimal 5 MB.</p></label>
