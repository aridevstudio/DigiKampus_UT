<x-layouts.admin title="Kelola Modul Kursus">
    ...
    <div id="addModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-2xl shadow-2xl">
                ...
                <form id="addMaterialForm" action="{{ route('admin.material.store', $course['id']) }}" method="POST" class="p-6" onsubmit="return handleAddMaterialSubmit(event)" x-data="{ isLoading: false }" @submit="isLoading = true">
                    @csrf
    ...
    <div id="editModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-2xl shadow-2xl">
                ...
                <form id="editForm" method="POST" class="p-6" x-data="{ isLoading: false }" @submit="isLoading = true">
                    @csrf
                    @method('PUT')
            ...
            <div class="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-6">
                <div class="text-center">
                    ...
                    <div class="flex justify-center gap-3">
                        ...
                        <form id="deleteForm" method="POST" class="inline" x-data="{ isLoading: false }" @submit="isLoading = true">
                            @csrf
                            @method('DELETE')
    ...
    <script>
        ...
        const typedContentRoutes = {
            video: @json(route('admin.kelola-video')),
            bacaan: @json(route('admin.kelola-bacaan')),
            kuis: @json(route('admin.kelola-quiz')),
            tugas: @json(route('admin.kelola-tugas')),
        };
        ...
        if(el) {
            var sortable = Sortable.create(el, {
                ...
                onEnd: function (evt) {
                    ...
                    // Send to server
                    fetch(`{{ route('admin.material.reorder', $course['id']) }}`, {
                        method: 'PUT',
                        headers: {
        ...
        function openEditModal(id) {
            fetch(`/admin/kursus/${courseId}/material/${id}`)
                .then(res => res.json())
                .then(data => {
                    document.getElementById('editForm').action = `/admin/kursus/${courseId}/material/${id}`;
        ...
        function confirmDelete(id) {
            document.getElementById('deleteForm').action = `/admin/kursus/${courseId}/material/${id}`;
