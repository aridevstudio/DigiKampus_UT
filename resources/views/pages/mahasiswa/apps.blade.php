<x-layouts.dashboard :active="'apps'">
    ...
    <div class="space-y-6 sm:space-y-8" x-data="{ query: '' }">
                ...
                <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                    <div class="max-w-2xl">
                        ...
                        <p class="mt-2 text-sm text-white/85 sm:text-base lg:text-lg">
                            Semua tools pembelajaranmu ada di sini. Klik kartu untuk membuka aplikasi — sistem akan membukanya sesuai mode masing-masing aplikasi.
                        </p>
        ...
        @if($visibleTotal === 0)
            <section class="rounded-2xl border border-dashed border-gray-300 bg-white p-10 text-center dark:border-gray-700 dark:bg-gray-800 sm:p-14" data-empty-state>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    @if($totalApps === 0)
                        Admin belum menambahkan aplikasi di panel Apps Management. Hubungi admin jika ini tidak sesuai.
                    @else
                        {{ $totalApps }} aplikasi terdaftar tetapi tidak ada yang aktif atau diizinkan untuk role Anda.
                    @endif
                </p>
            </section>
        @else
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                @foreach($items as $app)
                    @php
                        ...
                        $linkAttrs = $isNewTab ? 'target="_blank" rel="noopener noreferrer"' : '';
                    @endphp
