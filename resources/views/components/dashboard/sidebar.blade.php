...
@php
    // Struktur menu TIDAK berubah urutan. Hanya normalisasi styling & perbaikan bug
    // active detection untuk entry yang punya route "saudara" dengan suffix wildcard.
    $menuItems = [
    ...
    // Route DETIK yang SENGJA di-highlight oleh item ini di luar route utamanya sendiri.
    // Dipakai sebagai sumber tunggal untuk active state — collapsed dan expanded
    // membaca nilai yang sama, sehingga highlight konsisten di kedua mode.
    $extraActiveRoutes = [
        // "Courses" tetap aktif untuk halaman kursus, quiz, assignment.
        'courses'   => ['mahasiswa.course-*', 'mahasiswa.quiz-*', 'mahasiswa.assignment-*'],
        // "Bootcamp" TIDAK BOLEH match `bootcamp-saya` (entry sendiri di sidebar).
        'bootcamp'  => ['mahasiswa.bootcamp', 'mahasiswa.bootcamp-detail', 'mahasiswa.bootcamp-learn', 'mahasiswa.bootcamp.*'],
        // "Finance" aktif untuk halaman detail transaksi paralel.
        'finance'   => ['mahasiswa.finance', 'mahasiswa.transaction-detail'],
    ];

    // Tentukan "route bootcamp-saya apa pun" sekali untuk pengecualian.
    $isOnBootcampSaya = request()->routeIs('mahasiswa.bootcamp-saya', 'mahasiswa.bootcamp-saya.*');
...
{{-- Sidebar — modern, single-source active state --}}
<aside id="sidebar"
       aria-label="Menu navigasi utama mahasiswa"
       class="fixed left-0 top-0 h-screen bg-white dark:bg-[#1f2937] border-r border-gray-100 dark:border-gray-700/50 flex flex-col z-40 -translate-x-full md:translate-x-0 w-64 transition-[width,transform] duration-300 ease-in-out">

    {{-- Logo + mobile close --}}
    <div class="sidebar-logo-wrap h-16 px-4 border-b border-gray-100 dark:border-gray-700/50 flex items-center justify-between transition-all duration-300 ease-in-out">
        <img
            ...
            alt="SALUT Logo"
            class="sidebar-logo-image h-10 object-contain transition-all duration-300 ease-in-out"
        >
        <button onclick="toggleSidebar()" class="md:hidden p-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/50 rounded-lg transition-colors duration-200" aria-label="Tutup menu">
    ...
    <div class="px-4 py-4 border-b border-gray-100 dark:border-gray-700/50">
        <div class="sidebar-user-wrap flex items-center gap-3 transition-all duration-300 ease-in-out">
            ...
            @endif
            <div class="sidebar-user-meta min-w-0 transition-all duration-300 ease-in-out">
    ...
    {{-- Navigation Menu --}}
    <nav class="min-h-0 flex-1 overflow-y-auto overscroll-contain py-4 transition-all duration-300 ease-in-out" aria-label="Menu navigasi">
        <ul class="space-y-2 px-3">
            @foreach($menuItems as $item)
                @php
                    // ===== Sumber tunggal active state (digunakan IDENTIK oleh mode expanded & collapsed) =====
                    $isActive = request()->routeIs($item['route']) || request()->routeIs($item['route'] . '.*');

                    if (! $isActive && isset($extraActiveRoutes[$item['key']])) {
                        foreach ($extraActiveRoutes[$item['key']] as $candidate) {
                    ...
                    // Pengecualian: "Bootcamp" tidak boleh aktif saat user ada di halaman Bootcamp Saya.
                    if ($item['key'] === 'bootcamp' && $isOnBootcampSaya) {
                        $isActive = false;
                    }

                    // Class untuk icon: putih saat aktif, slate-500 saat non-aktif (hover naik ke slate-900).
                    $iconClass = $isActive
                        ? 'text-white'
                ...
                <li>
                    <a
                        ...
                        class="group flex items-center h-12 px-3 rounded-2xl text-sm transition-all duration-300 ease-in-out
                            {{ $isActive
                    ...
                    >
                        @switch($item['icon'])
                            @case('home')
                                <svg class="w-5 h-5 flex-shrink-0 transition-colors duration-300 ease-in-out {{ $iconClass }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
                            ...
                            @case('newspaper')
                                ...
                                @break
                        @endswitch
                        <span class="sidebar-label ml-3 whitespace-nowrap overflow-hidden transition-all duration-300 ease-in-out">{{ $item['name'] }}</span>
                    </a>
                </li>
            @endforeach
        </ul>
